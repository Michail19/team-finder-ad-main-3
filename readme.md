...
TeamFinder — веб-приложение на Django для поиска единомышленников и совместной работы над проектами.  
Пользователи могут создавать профиль, публиковать проекты, присоединяться к чужим проектам, добавлять проекты в избранное и просматривать других участников платформы.
...
```bash
git clone https://github.com/Michail19/team-finder-ad-main-3.git
...
**ВАЖНО!** Для проекта использовался вариант 1 шаблона, поэтому в TASK_VERSION для корректной работы необходимо указать 1: `TASK_VERSION=1`.
...
Для работы проекта необходима запущенная на компьютере база данных PostgreSQL. Если вы не хотите её устанавливать, можете в `team_finder/settings.py` закомментировать блок:
...
Студент 3 курса группы ИКБО-14-23
